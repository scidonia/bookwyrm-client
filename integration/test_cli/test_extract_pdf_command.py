...
def test_extract_pdf_command_invalid_page_range():
    ...
    try:
        result = run_bookwyrm_command([
            'extract-pdf',
            '--file', str(test_file),
            '--start-page', '0'  # Invalid: pages are 1-based
        ])
...
def test_extract_pdf_command_negative_num_pages():
    ...
    try:
        result = run_bookwyrm_command([
            ...
            '--file', str(test_file),
            '--start-page', '1',
            '--num-pages', '-1'  # Invalid: negative pages
        ])
...
def test_extract_pdf_command_live_api_streaming(api_key, api_url):
    ...
    try:
        ...
        assert ('page' in result.stdout.lower() or 
               'progress' in result.stdout.lower() or
               'extracted' in result.stdout.lower() or
...
def test_extract_pdf_command_live_api_url_source(api_key, api_url):
    ...
    if result.returncode == 0:
        assert len(result.stdout) > 0
        assert ('page' in result.stdout.lower() or 
               'extracted' in result.stdout.lower())
    else:
