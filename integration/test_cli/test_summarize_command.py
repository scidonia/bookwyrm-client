...
def test_summarize_command_negative_max_tokens():
    ...
    result = run_bookwyrm_command([
        'summarize',
        'test.jsonl',  # File doesn't need to exist for this validation test
        '--max-tokens', '-1000'  # Invalid: negative
    ])
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            assert ('empty' in result.stderr.lower() or 
                   'api' in result.stderr.lower() or
                   'key' in result.stderr.lower())
...
def test_summarize_command_large_max_tokens(sample_phrases):
    ...
    try:
        result = run_bookwyrm_command([
            'summarize',
            str(jsonl_file),
            '--max-tokens', '100000'  # Very large value
        ])
...
@pytest.mark.liveonly
def test_summarize_command_live_api_streaming(sample_scientific_phrases, api_key, api_url):
    ...
    try:
        ...
        assert ('progress' in result.stdout.lower() or 
...
def test_summarize_command_live_api_with_debug(sample_phrases, api_key, api_url):
    ...
    try:
        ...
        assert ('debug' in result.stdout.lower() or 
               'level' in result.stdout.lower() or
...
@pytest.mark.liveonly
def test_summarize_command_live_api_no_stream(sample_scientific_phrases, api_key, api_url):
...
@pytest.mark.liveonly
def test_summarize_command_live_api_large_content(sample_long_content, api_key, api_url):
    ...
    try:
        result = run_bookwyrm_command([
            ...
            '--api-key', api_key,
            '--base-url', api_url,
            '--max-tokens', '3000',  # Smaller chunks to test hierarchical summarization
            '--debug'
        ])
        ...
        assert ('artificial' in result.stdout.lower() or 
               'intelligence' in result.stdout.lower() or
               'machine' in result.stdout.lower() or
               'learning' in result.stdout.lower())
...
@pytest.mark.liveonly
def test_summarize_command_live_api_comprehensive_options(sample_phrases, api_key, api_url):
