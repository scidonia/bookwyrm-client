...
def test_classify_command_with_filename_hint(sample_python_content):
    ...
    try:
        result = run_bookwyrm_command([
            'classify',
            '--file', str(test_file),
            '--filename', 'script.py'  # Correct hint
        ])
...
def test_classify_command_live_api_direct_content(api_key, api_url):
    ...
    assert ('classification' in result.stdout.lower() or 
           'format' in result.stdout.lower() or
           'type' in result.stdout.lower())
...
def test_classify_command_live_api_binary_file(api_key, api_url):
    ...
    try:
        ...
        assert ('image' in result.stdout.lower() or 
               'png' in result.stdout.lower() or
               'binary' in result.stdout.lower())
...
@pytest.mark.liveonly
def test_classify_command_live_api_with_filename_hint(sample_csv_content, api_key, api_url):
    ...
    try:
        result = run_bookwyrm_command([
            'classify',
            '--file', str(test_file),
            '--filename', 'data.csv',  # Correct hint
            '--api-key', api_key,
            '--base-url', api_url
        ])
        ...
        assert ('csv' in result.stdout.lower() or 
               'comma' in result.stdout.lower())
