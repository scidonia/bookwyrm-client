...
def test_get_citations_quality_scores(client, sample_chunks):
    ...
    for citation in response.citations:
        assert isinstance(citation.quality, int)
        assert 0 <= citation.quality <= 4, f"Quality score {citation.quality} is out of range [0-4]"
...
def test_get_citations_live_api_comprehensive(client, sample_chunks, sample_jsonl_content):
    """Comprehensive test of citation finding against live API."""
    # Test 1: Basic citation finding with chunks
    response1 = client.get_citations(
        chunks=sample_chunks,
        question="What is machine learning and how does it relate to AI?",
        max_tokens_per_chunk=1000
    )
    assert isinstance(response1, CitationResponse)
    assert response1.total_citations >= 0

    # Test 2: Citation finding with JSONL content
    response2 = client.get_citations(
        jsonl_content=sample_jsonl_content,
        question="Who created Python and when?",
        start=0,
        limit=5
    )
    assert isinstance(response2, CitationResponse)
    assert response2.total_citations >= 0

    # Test 3: Streaming citations
    citations = []
    for response in client.stream_citations(
        chunks=sample_chunks[:3],  # Limit for test speed
        question="What is natural language processing?",
        max_tokens_per_chunk=800
    ):
        if isinstance(response, CitationStreamResponse):
            citations.append(response.citation)
            if len(citations) >= 2:  # Limit for test speed
                break

    # Test 4: Citation finding with pagination
    response4 = client.get_citations(
        chunks=sample_chunks,
        question="What are the different AI technologies?",
        start=1,
        limit=3,
        max_tokens_per_chunk=500
    )
    assert isinstance(response4, CitationResponse)

    # Test 5: Different chunk sizes
    response5 = client.get_citations(
        chunks=sample_chunks[:2],
        question="What is deep learning?",
        max_tokens_per_chunk=200
    )
    assert isinstance(response5, CitationResponse)
