...
def test_stream_process_text_with_offsets_flag(client):
    ...
    for phrase in text_span_results:
        ...
        assert phrase.end_char > phrase.start_char, "end_char should be greater than start_char"
        assert phrase.text == text[phrase.start_char:phrase.end_char], "Text should match slice"
...
def test_stream_process_text_with_text_only_flag(client):
    ...
    for phrase in phrases:
        assert isinstance(phrase, TextResult), "Should be TextResult without position info"
...
def test_stream_process_text_with_chunk_size_and_offsets(client):
    ...
    for chunk in chunks:
        assert len(chunk.text) <= 1200, f"Chunk too large: {len(chunk.text)} chars"  # Allow some flexibility
...
def test_stream_process_text_from_url_with_chunk_size(client):
    ...
    # Using Project Gutenberg URL as in README example
    url = "https://www.gutenberg.org/cache/epub/32706/pg32706.txt"  # Triplanetary by E. E. Smith
    ...
    # Verify we got results from the URL
    assert len(phrases) > 0, "Should have extracted phrases from URL"
    assert progress_count > 0, "Should have received progress updates"
    
    # Verify chunks are reasonably sized
    for phrase in phrases[:10]:  # Check first 10 to avoid long test times
        assert len(phrase.text) <= 1200, f"Phrase too large: {len(phrase.text)} chars"


def test_stream_process_text_url_text_only(client):
    """Test phrasal processing from URL with text_only format."""
    url = "https://www.gutenberg.org/cache/epub/32706/pg32706.txt"
    ...
    for response in client.stream_process_text(
        ...
        if isinstance(response, (TextResult, TextSpanResult)):
            phrases.append(response)
            if len(phrases) >= 5:  # Limit to first 5 for test speed
                break
...
def test_stream_process_text_response_format_enum(client):
    ...
    text_span_results = [p for p in phrases_with_offsets if isinstance(p, TextSpanResult)]
    assert len(text_span_results) > 0, "WITH_OFFSETS should include position info"
    
    for phrase in phrases_text_only:
        assert isinstance(phrase, TextResult), "TEXT_ONLY should not include position info"
...
def test_stream_process_text_live_api_comprehensive(client):
    """Comprehensive test of phrasal processing against live API."""
    # Test various combinations to ensure they all work with live API
    test_text = "Machine learning is a method of data analysis that automates analytical model building. It is a branch of artificial intelligence based on the idea that systems can learn from data, identify patterns and make decisions with minimal human intervention."
    
    # Test 1: Basic offsets
    phrases_1 = []
    for response in client.stream_process_text(text=test_text, offsets=True):
        if isinstance(response, (TextResult, TextSpanResult)):
            phrases_1.append(response)
    assert len(phrases_1) > 0
    
    # Test 2: Text only
    phrases_2 = []
    for response in client.stream_process_text(text=test_text, text_only=True):
        if isinstance(response, (TextResult, TextSpanResult)):
            phrases_2.append(response)
    assert len(phrases_2) > 0
    
    # Test 3: With chunk size
    phrases_3 = []
    for response in client.stream_process_text(text=test_text, chunk_size=100, offsets=True):
        if isinstance(response, (TextResult, TextSpanResult)):
            phrases_3.append(response)
    assert len(phrases_3) > 0
    
    # Test 4: URL processing (limited to avoid long test times)
    url_phrases = []
    count = 0
    for response in client.stream_process_text(
        ...
        if isinstance(response, (TextResult, TextSpanResult)):
            url_phrases.append(response)
            count += 1
            if count >= 3:  # Limit for test speed
                break
    assert len(url_phrases) > 0
