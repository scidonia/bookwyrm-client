...
class TextSpan(Text, Span):
    ...
    TextSpan is a fundamental model that combines text content with its position
    within a larger document. It's used throughout the BookWyrm API to represent
    chunks of text that maintain their original location context.

    This model is essential for:
    - Citation finding: Chunks of text that can be searched for relevant citations
    - Text processing: Phrases or chunks extracted from larger documents
    - PDF extraction: Text elements with their position in the original document
    - Summarization: Input phrases that maintain their source location

    The position information (start_char, end_char) represents character offsets
    in the original source text, enabling precise text location and reconstruction.

    Attributes:
        text: The actual text content of this span
        start_char: Zero-based starting character position in the source document
        end_char: Zero-based ending character position in the source document (exclusive)

    Examples:
        Creating a TextSpan for citation finding:
        ...
        Using TextSpans from PDF extraction:
        
        ```python
        # Extract text from PDF
        pdf_response = client.extract_pdf(pdf_bytes=pdf_data)
        
        # Convert PDF elements to TextSpans for citation finding
        text_spans = []
        for page in pdf_response.pages:
            for element in page.text_blocks:
                span = TextSpan(
                    text=element.text,
                    start_char=element.coordinates.x1,  # Using PDF coordinates
                    end_char=element.coordinates.x2
                )
                text_spans.append(span)
        ```

        Using TextSpans from text processing:
        
        ```python
        # Process text into chunks
        chunks = []
        for response in client.stream_process_text(text=document, chunk_size=1000):
            if isinstance(response, TextSpanResult):
                # Convert to TextSpan for citation finding
                span = TextSpan(
                    text=response.text,
        ...
        # Use chunks for citation finding
        citations = client.get_citations(chunks=chunks, question="What is the main topic?")
        ```

    Note:
        TextSpan is the input format for citation finding and summarization.
        TextSpanResult is the output format from text processing operations.
        Both share the same structure but serve different roles in the API workflow.
    """
